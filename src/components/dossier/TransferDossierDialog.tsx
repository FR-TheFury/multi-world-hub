...
const TransferDossierDialog = ({ 
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('transfer-dossier', {
        body: {
          action: 'initiate_transfer',
          dossierId: dossierId,
          targetWorldCode: targetWorld
        }
      });
      ...
      if (data?.success) {
        toast.success(data.message || 'Dossier transféré avec succès');
        onOpenChange(false);
        
        // Navigate to the new dossier
        if (data.new_dossier_id) {
          setTimeout(() => {
            navigate(`/dossier/${data.new_dossier_id}`);
          }, 1000);
        }
      } else {
